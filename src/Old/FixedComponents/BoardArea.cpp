...
#include "Game/FixedComponents/BoardArea.hpp"
...
void BoardArea::displayDescription(Coordinates id)
{
...
void BoardArea::handleClicked(Coordinates id)
{
	board_->tileClicked(id);
}

void BoardArea::executeMove(const QList <Coordinates> &path)
{
	pathToAnimate = path;
...
void BoardArea::highlightFields(const QList <Coordinates> &fieldsIDs)
{
	while (!highlightedFields_.isEmpty()) {
		tiles_[highlightedFields_.back()]->setHighlighted(false);
		highlightedFields_.pop_back();
	...
	for (auto id : fieldsIDs) {
		int idx = board_->coordinatesToIndex(id);
...
{
	const Coordinates field = board_->player(index)->position();
...
QPointF BoardArea::calcCenter(const Coordinates id, qreal side)
{
