...
{
	wounds = player->maxHealth() - player->currentHealth();
...
{
	int toHeal = player_->currentHealth() + HealingPerGoldPiece * goldPaid;
	healthAfterHealing = qMin(player_->maxHealth(), toHeal);
	...
	text += QString("/");
	text += QString::number(player_->maxHealth());
