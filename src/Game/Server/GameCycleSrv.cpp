...
#include "GameCycleSrv.hpp"

#include "Core/Utils/Serial.hpp"

GameCycleSrv::GameCycleSrv(ConnectionAdapterSrv *connAdapter) :
	connAdapter_(connAdapter)
{
...
// 	connect(&newGameSrv_, &NewGameSrv::gameSet, this, &GameCycleSrv::beginGame);
}

void GameCycleSrv::beginGame(const QHash <UID, PlayerDraft> &playersDrafts)
{
// 	disconnect(connAdapter_, &ConnectionAdapterSrv::newUser, newGameSrv_, &NewGameSrv::onUserEntered);
// 	disconnect(connAdapter_, &ConnectionAdapterSrv::userDisconnected, newGameSrv_, &NewGameSrv::onUserQuit);
